/**
 * Utilidades de validación para nombres de categorías y productos en el backend.
 */
 ...
 * @returns Un mensaje de error si no es válido, o null si es completamente válido.
 */
export function validateName(name: string | undefined | null): string | null {
  ...
  // 2. No se permiten caracteres repetidos 3 o más veces consecutivamente
  if (THREE_CONSECUTIVE_IDENTICAL.test(trimmed)) {
    return 'No se permiten caracteres repetidos consecutivamente.';
  }

  // 3. Dividir en palabras para análisis de "gibberish" / "keyboard mashing"
...
/**
 * Valida la descripción de una categoría o producto en el backend.
...
export function validateDescription(description: string | undefined | null): string | null {
  ...
  if (!SPANISH_DESCRIPTION_ALLOWED.test(trimmed)) {
    return 'La descripción solo puede contener letras, números y signos de puntuación comunes (sin símbolos como @, #, $, etc.).';
  }
