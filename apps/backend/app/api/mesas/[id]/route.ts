...
// 🛡️ NUEVO: Método GET para obtener UNA SOLA mesa por su ID al instante sin saturar la BD
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ error: 'Error interno del servidor al obtener la mesa' }, { status: 500 });
  }
}

// El PATCH original con la protección de transacciones
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [mesa] = await prisma.$transaction(async (tx) => {
      ...
      maxWait: 10000, // 10 segundos máximo esperando conexión libre
      timeout: 15000  // 15 segundos máximo para terminar la transacción
    });
