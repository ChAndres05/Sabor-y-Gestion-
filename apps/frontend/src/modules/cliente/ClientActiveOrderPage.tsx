...
export default function ClientActiveOrderPage({ user, tableId, onBack }: ClientActiveOrderPageProps) {
  ...
  const refreshOrders = useCallback(async () => {
    ...
    if (ordersData.length > 0) {
      const clientOrder = ordersData.find((activeOrder) => activeOrder.customer?.idUsuario === user.id) ?? ordersData[0];
  ...
  const refreshPageState = useCallback(() => {
    ...
    refreshPageStateTimer.current = setTimeout(() => {
      void Promise.all([refreshOrders(), refreshTable()]);
    }, 300);
  ...
  useEffect(() => {
    ...
    const handleTableOrderUpdated = (updatedOrder: { id_mesa?: number | string; tableId?: number | string }) => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const currentOrder = ordersData.find((activeOrder) => activeOrder.customer?.idUsuario === user.id) ?? ordersData[0] ?? null;
        setOrder(currentOrder);

        if (!currentOrder && tableData?.estado !== 'FUERA_DE_SERVICIO') {
          const payload = { nombre: `${user.nombre} ${user.apellido}`, telefono: user.telefono || '00000000', correo: user.correo || '', ci: '', idUsuario: user.id };
  ...
  const handleStartEditItem = (item: TableOrderItem) => {
    skipNextIngredientHydration.current = selectedCategoryId !== item.categoriaId || selectedProductId !== item.productoId;
    ...
    setIngredientSelections((item.ingredientes ?? []).map((ingredient, index) => ({ id: index + 1, nombre: ingredient.nombre, incluido: ingredient.incluido, incluidoPorDefecto: ingredient.incluido })));
    setIsItemModalOpen(true);
  };
  ...
  const handleChangeOrderStatus = async (status: TableOrderStatus) => {
    ...
    try {
      if (order) await ordersApi.updateOrderStatus(order.id, status, tableId);
      refreshPageState();
      setActiveStep('pedido');
  ...
  const handleToggleIngredient = (ingredientId: number) => {
    setIngredientSelections((current) => current.map((ingredient) => ingredient.id === ingredientId ? { ...ingredient, incluido: !ingredient.incluido } : ingredient));
  };
                  ...
                  {products.map((product) => (
                    <article key={product.id} className="rounded-2xl bg-white p-4 shadow-sm flex flex-col justify-between">
                      <div className="grid grid-cols-[48px_1fr_auto] gap-3">
                        ...
                        <button type="button" onClick={() => openProductModal(product)} disabled={!canEditItems} className="self-start rounded-xl bg-primary px-3 py-2 text-[12px] font-bold text-white disabled:opacity-60">+ Pedir</button>
                      </div>
                    </article>
                  ))}
                </div>
            ...
            {activeStep === 'pedido' && (
              <section className="space-y-4">
                <div className="rounded-[1.5rem] bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <h2 className="text-[20px] font-bold text-text">Estado de tu pedido</h2>
                      <p className="mt-1 text-[13px] leading-5 text-gray-500">Mesa {table?.numero ?? tableId}</p>
                    </div>
                  </div>
                  
                  {order && (
                    <div className="mt-4 border-t border-gray-100 pt-4">
                      <div className="flex items-center justify-between mb-3"><span className="text-[14px] font-bold text-gray-500">Fase actual</span><span className={`rounded-full px-3 py-1 text-[11px] font-bold ${getStatusBadgeClass(order.estado)}`}>{getOrderStatusLabel(order.estado)}</span></div>
                ...
                {!order || (order.items?.length ?? 0) === 0 ? (
                  <div className="rounded-[1.5rem] bg-white p-6 text-center shadow-sm">
                    <p className="font-bold text-text">Aún no has agregado platos</p>
                    <button type="button" onClick={() => setActiveStep('menu')} className="mt-4 rounded-xl bg-primary px-5 py-3 text-[13px] font-bold text-white">Ver el menú</button>
                  </div>
                ) : (
                  <>
                    {canEditItems && (
                    ...
                    <div className="rounded-[1.5rem] bg-white p-5 shadow-sm">
                      ...
                      <p className="mt-1 text-[12px] font-medium text-gray-500">Tiempo aproximado: {order.tiempoEstimadoMinutos} min <span aria-hidden="true">&middot;</span> Platos: {order.items.length}</p>
                    </div>
                    ...
                    {isBillRequested && <div className="rounded-[1.5rem] bg-info/10 p-4 text-[13px] font-bold text-info">Has solicitado la cuenta. Un mesero te atenderá enseguida.</div>}
                  </>
                )}
              </section>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6">
          <section className="max-h-[92vh] w-full max-w-[390px] overflow-y-auto rounded-[1.5rem] bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div><h2 className="text-[20px] font-bold text-text">{editingItemId ? 'Editar plato' : 'Personalizar'}</h2><p className="mt-1 text-[13px] leading-5 text-gray-500">Ajusta las cantidades y los ingredientes de tu plato.</p></div>
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-2"><label className="text-[13px] font-bold text-text">Plato seleccionado</label><select value={selectedProductId} onChange={(e) => setSelectedProductId(Number(e.target.value))} disabled className="rounded-xl border border-gray-300 bg-gray-50 px-3 py-3 text-[14px] outline-none opacity-80">{products.map((product) => (<option key={product.id} value={product.id}>{product.nombre}</option>))}</select></div>
              <div className="grid grid-cols-[1fr_90px] gap-3">
                <div className="flex flex-col gap-2"><label className="text-[13px] font-bold text-text">Observación especial</label><input type="text" value={observation} onChange={(e) => setObservation(e.target.value)} placeholder="Ej. Poco picante, sin sal" className="rounded-xl border border-gray-300 bg-white px-3 py-3 text-[14px] outline-none focus:border-primary" /></div>
              ...
              <div>
                ...
                {removedFromCurrentSelection.length > 0 && <p className="text-[12px] font-bold text-alert mt-2">La cocina lo preparará: {removedFromCurrentSelection.map((ingredient) => `sin ${ingredient.nombre.toLowerCase()}`).join(', ')}</p>}
              </div>
              <div className="grid grid-cols-2 gap-3 pt-2">
                ...
                <button type="button" onClick={() => void handleSaveItem()} disabled={isSavingItem || !selectedProductId || !quantity || Number(quantity) < 1} className="rounded-xl bg-primary px-4 py-3 text-[13px] font-bold text-white disabled:opacity-60">{isSavingItem ? 'Guardando...' : editingItemId ? 'Actualizar' : 'Agregar'}</button>
              </div>
            </div>
          </section>
        </div>
