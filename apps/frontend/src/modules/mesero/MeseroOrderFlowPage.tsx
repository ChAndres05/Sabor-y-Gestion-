...
export default function MeseroOrderFlowPage({ user, tableId, onBack, onOpenOrders }: MeseroOrderFlowPageProps) {
  ...
  const handleSearchCustomer = async () => {
    ...
    } catch (error) { console.error(error); setFeedback({ type: 'error', title: 'Error de búsqueda', message: 'No se pudo conectar con el servidor.' }); } finally { setIsSearchingCustomer(false); }
  };
  ...
  const handleSaveCustomer = async () => {
    ...
    } catch (error) { console.error(error); setFeedback({ type: 'error', title: 'No se pudo guardar', message: error instanceof Error ? error.message : 'Ocurrió un error inesperado' }); } finally { setIsSavingCustomer(false); }
  };
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      const payload: AddOrderItemPayload = {
        ...
        ...(selectedProduct?.presentacionId ? { presentacionId: selectedProduct.presentacionId } : {}),
        ...(selectedProduct?.nombre ? { productoNombre: selectedProduct.nombre } : {}),
        cantidad: Number(quantity),
        observacion: observation,
        ingredientes: ingredientSelections.map((ingredient) => ({ nombre: ingredient.nombre, incluido: ingredient.incluido })),
  ...
  const handleChangeOrderStatus = async (status: TableOrderStatus) => {
    ...
    try {
      ...
      else await ordersApi.updateOrderStatus(0, status, tableId);
  ...
  const handleNewOrder = async () => {
    ...
    try {
      ...
      setOrder(newOrder);
      setActiveStep('menu');
      setFeedback({ type: 'success', title: 'Nuevo pedido', message: 'Se ha creado un nuevo pedido para esta mesa.' });
    } catch (error) { console.error(error); setFeedback({ type: 'error', title: 'Error', message: error instanceof Error ? error.message : 'No se pudo crear' }); } finally { setIsLoading(false); }
  };

  const handleRequestBill = async () => {
    // 🛑 NUEVA REGLA (FRONTEND BLOQUEO): Antes de mandar nada al backend
    if (hasPendingOrders) {
      setFeedback({
        type: 'error',
        title: 'Acción Denegada',
        message: 'No puedes solicitar la cuenta. Aún hay pedidos en preparación para esta mesa.'
      });
      return;
    }
    ...
    try {
      ...
      refreshPageState();
      setFeedback({ type: 'success', title: 'Cuenta solicitada', message: 'La mesa quedó en cuenta solicitada y lista para caja.' });
    } catch (error) { console.error(error); setFeedback({ type: 'error', title: 'Error', message: error instanceof Error ? error.message : 'No se pudo solicitar cuenta' }); } finally { setIsRequestingBill(false); }
  };
                        ...
                        <article key={product.id} className="rounded-2xl bg-white p-4 shadow-sm flex flex-col justify-between">
                          <div className="grid grid-cols-[48px_1fr_auto] gap-3">
                            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-background text-[22px] overflow-hidden">
                              {typeof product.imagen === 'string' && product.imagen.trim() && (product.imagen.startsWith('http') || product.imagen.startsWith('/') || product.imagen.includes('.')) ? (
                            ...
                            <div>
                              ...
                              <p className="mt-1 text-[12px] font-bold text-primary">{formatCurrency(product.precio)} · {product.tiempoPreparacion} min</p>
                            </div>
                ...
                <div className="rounded-[1.5rem] bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-[13px] leading-5 text-gray-500">Mesa {table?.numero ?? tableId} · {activeOrders.length} pedido(s)</p>
                    </div>
                  </div>
                  ...
                  <>
                    {canEditItems && (
                      <div className="flex justify-end">
                        <button type="button" onClick={() => setActiveStep('menu')} className="flex h-12 w-12 items-center justify-center rounded-full bg-primary text-[28px] font-bold leading-none text-white shadow-md hover:bg-primary/90" aria-label="Agregar más productos">+</button>
                      </div>
                    )}
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                              ...
                              <div className="flex gap-1">
                                {canEditItems && (
                                  <>
                                    ...
                                    <button type="button" onClick={() => void handleRemoveItem(item.id)} className="h-8 w-8 rounded-lg bg-alert/10 text-[15px] font-bold text-alert">🗑</button>
                                  </>
                                )}
                              </div>
                            </div>
                    ...
                    <div className="rounded-[1.5rem] bg-white p-5 shadow-sm">
                      ...
                      <p className="mt-1 text-[12px] font-medium text-gray-500">Tiempo estimado: {order.tiempoEstimadoMinutos} min · Items: {order.items.length}</p>
                    </div>
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:flex-wrap">
                        ...
                        {order.estado === 'ENTREGADO' && (
                          <button
                            ...
                            disabled={isRequestingBill || hasPendingOrders}
                            className={`w-full md:w-auto flex-1 rounded-xl px-5 py-3 text-[14px] font-bold text-white transition-all ${hasPendingOrders ? 'bg-gray-300 cursor-not-allowed opacity-80' : 'bg-primary hover:bg-primary/90'
                              }`}
                          >
                            {hasPendingOrders ? 'Aún hay pedidos en cocina' : (isRequestingBill ? 'Solicitando...' : 'Solicitar cuenta')}
                          </button>
                        )}
                      </div>
                    )}
          ...
          <section className="max-h-[92vh] w-full max-w-[390px] overflow-y-auto rounded-[1.5rem] bg-white p-5 shadow-2xl">
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-2">
                ...
                <select value={selectedCategoryId} onChange={(e) => setSelectedCategoryId(Number(e.target.value))} className="rounded-xl border border-gray-300 bg-white px-3 py-3 text-[14px] outline-none focus:border-primary">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select value={selectedProductId} onChange={(e) => setSelectedProductId(Number(e.target.value))} className="rounded-xl border border-gray-300 bg-white px-3 py-3 text-[14px] outline-none focus:border-primary">
              ...
              <div className="grid grid-cols-[1fr_90px] gap-3">
                <div className="flex flex-col gap-2">
                  ...
                  <input type="text" value={observation} onChange={(e) => setObservation(e.target.value)} placeholder="Ej. Sin locoto" className="rounded-xl border border-gray-300 bg-white px-3 py-3 text-[14px] outline-none focus:border-primary" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-text">Cantidad</label>
                  <input type="number" min="1" value={quantity} onChange={(e) => { const value = e.target.value; if (value === '' || Number(value) > 0) setQuantity(value); }} onBlur={(e) => { if (!e.target.value || Number(e.target.value) < 1) setQuantity('1'); }} className="rounded-xl border border-gray-300 bg-white px-3 py-3 text-center text-[14px] outline-none focus:border-primary" />
                </div>
              </div>
